...
# following is for the university page
    ...
    def show_subfield_overview():
        ...
        col1, gap1, col2, gap2= st.columns([5.5,0.5,2.5, 0.5])
        with col1:
            #st.markdown('\n')
            #st.markdown('\n')
            #st.markdown('\n')
    ...
    def show_research_comparison():
        ...
        col1, gap2, col2 = st.columns([4,0.2, 2])
        with col1:
...
#-----------------------------------------------------------------------------------------
    ...
    sections_prof_page = {
        ...
        "📊  Publication Metrics": lambda: academic_metrics(),
        "🔬  Research Profile": lambda: research_profile(),
        "📑  Comparison": lambda: comparison(),
        "❓  Questions?": lambda: questions()
    }
    if "selected_prof_page_section" not in st.session_state:
        st.session_state.selected_prof_page_section = list(sections_prof_page.keys())[0]
    ...
    cols = st.columns(len(sections_prof_page))
    for i, section in enumerate(sections_prof_page.keys()):
        if cols[i].button(section, use_container_width=True):
            st.session_state.selected_section = section

    st.divider()
    sections_prof_page[st.session_state.selected_section]()



elif selected_section == "Topic overview":
    st.header("Research Topics in Biomedical Engineering")
    st.write('\n')
    st.divider() 
    st.subheader("1. Overview of the 20 Subtopics in Biomedical Engineering")

    st.markdown(
        """
        🗝️ In this section, we present **20 key subtopics** in **Biomedical Engineering**, identified using **unsupervised machine learning** for topic modeling.  
        >note: Each subtopic is represented as a **word cloud**, highlighting the most relevant keywords that define its theme.  
